...
/// Credential-agnostic helpers for contexts where the full `EngageKaro.shared`
...
/// and can't await the async, actor-isolated API on `EngageKaro`.
///
/// Read your app's config from the App Group `UserDefaults` that
/// `EngageKaro.shared.shareConfig(...)` wrote at login (keys `ek_api_base`,
/// `ek_api_key`, `ek_external_id`), and pass them here directly.
public enum EngageKaroReceipts {
    ...
    public static func reportDeliveredBlocking(
        ...
        if let identityHash { req.setValue(identityHash, forHTTPHeaderField: "X-EngageKaro-Identity-Hash") }
